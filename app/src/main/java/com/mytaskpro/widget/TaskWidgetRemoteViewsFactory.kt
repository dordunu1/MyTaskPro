...
class TaskWidgetRemoteViewsFactory(
    ...
    companion object {
        ...
        private const val MAX_TASKS = 10 // Increased to 10 tasks
    }
    ...
    override fun onDataSetChanged() {
        runBlocking {
            try {
                val deferredTasks = coroutineScope.async {
                    taskDao.getUpcomingTasks(MAX_TASKS, Date(currentTime))
                }
                tasks = deferredTasks.await()
                Log.d(TAG, "Fetched ${tasks.size} tasks for widget")
            } catch (e: Exception) {
